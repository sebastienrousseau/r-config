...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/r-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Tidyverse-Aligned lintr & styler Standards](#tidyverse-aligned-lintr-styler-standards) — before and after rule comparison
...
Install `@sebastienrousseau/r-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/r-config

# pnpm
pnpm add -D @sebastienrousseau/r-config

# yarn
yarn add -D @sebastienrousseau/r-config

# bun
bun add -d @sebastienrousseau/r-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/r-config.git
cd r-config
make            # check + test
```
...
- **Target environment**: Optimized for R 4.3+ / lintr.
...
```bash
cp node_modules/@sebastienrousseau/r-config/.lintr ./.lintr
```
...
`@sebastienrousseau/r-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/r-config` solves this with three deliberate engineering choices:
...
## Tidyverse-Aligned lintr & styler Standards

Enforces consistent snake_case naming, line length limits, and vectorized programming best practices in R data pipelines.
...
### After (@sebastienrousseau/r-config Enforced)
...
When maintaining legacy S-Plus statistical routines incompatible with tidyverse idioms and lintr AST parsing.
